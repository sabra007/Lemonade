...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        //  - Additionally, lemonSize is no longer relevant and should be set to -1
            ...
            Log.i("In clickLemonImage B ++", "squeezeCount = $squeezeCount, lemonSize = $lemonSize")
            squeezeCount++
            Log.i("In clickLemonImage A ++", "squeezeCount = $squeezeCount, lemonSize = $lemonSize")
            lemonSize--
            if (lemonSize == 0) {
                lemonadeState = DRINK
                lemonSize = -1
    ...
    private fun setViewElements() {
        Log.i("In SetViewElement", "squeezeCount = $squeezeCount, lemonSize = $lemonSize")
        val textAction: TextView = findViewById(R.id.text_action)

        var res: String = ""
        if (lemonadeState == SELECT) {
            res = resources.getString(R.string.lemon_select)
            textAction.text = res
    ...
    private fun showSnackbar(): Boolean {
        Log.i("In showSnackbar", "squeezeCount = $squeezeCount, lemonSize = $lemonSize")
